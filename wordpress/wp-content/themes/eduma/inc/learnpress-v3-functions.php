		...
		<aside class="thim-course-filter-wrapper">
			<form action="" name="thim-course-filter" method="POST" class="thim-course-filter">
				...
				if ( get_theme_mod( 'thim_filter_by_cate', false ) ) {
					...
					$course_term = get_terms( apply_filters( 'thim_display_cate_filter', array( 'taxonomy' => 'course_category', 'parent' => 0 ) ) );
						...
						foreach ( $course_term as $term ) {
							...
							<?php if ( checked( $current_term_id, $term->term_id, false ) ): ?>
								<input type="checkbox" name="course-cate-filter"
									   id="<?php esc_attr_e( $input_id ); ?>" class="filtered"
									   value="<?php esc_attr_e( $term->term_id ); ?>" checked>
							<?php else: ?>
								<input type="checkbox" name="course-cate-filter"
							...
							if ( $terms_child && !is_wp_error( $terms_child ) ) {
								...
								foreach ( $terms_child as $child_id ) {
									...
									?>
									<li class="term-item">
										<?php if ( checked( $current_term_id, $child_id, false ) ): ?>
											<input type="checkbox" name="course-cate-filter"
												   id="<?php esc_attr_e( $input_child_id ); ?>" class="filtered"
												   value="<?php esc_attr_e( $child_id ); ?>" checked>
										<?php else: ?>
											<input type="checkbox" name="course-cate-filter"
										...
										<label for="<?php esc_attr_e( $input_child_id ) ?>">
											...
											<span><?php printf( esc_html__( '(%s)', 'eduma' ), $term_child->count ); ?></span>
										</label>
									</li>
								<?php }
								echo '</ul>';
