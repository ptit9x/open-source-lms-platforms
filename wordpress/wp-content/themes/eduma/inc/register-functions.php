...
class ThimEdumaRegisterFunction {
	...
	protected function addHook() {
		...
		add_filter( 'learn-press/add-to-cart-redirect', array( $this, 'url_redirect_after_checkout_lp_course_success' ), 10, 4 );
	...
	private function buyCourse() {
		if ( is_plugin_active( 'learnpress-woo-payment/learnpress-woo-payment.php' )
			...
			if ( isset( $_REQUEST['urlHandleAjax'] ) ) {
				$urlHandleAjax = sanitize_text_field( $_REQUEST['urlHandleAjax'] );

				foreach ( $_REQUEST as $k => $param ) {
					if ( $k == 'urlHandleAjax' ) {
						continue;
					}
					$urlHandleAjax = add_query_arg( array( $k => $param ), $urlHandleAjax );
...
You registered success on %s site
